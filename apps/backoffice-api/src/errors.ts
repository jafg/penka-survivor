 ...
 *
 * Deliberately a copy of @penka/api's handler rather than a shared import: an
 * app cannot import from another app, and the error envelope is the one thing
 * both must produce identically. What is shared is what makes that possible —
 * the `ErrorCodes` set and the `ApiError` schema in the contract.
 */
