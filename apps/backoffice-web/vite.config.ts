...
const checklistPath = fileURLToPath(
  new URL('../../docs/visual-parity-checklist.md', import.meta.url),
);
...
export default defineConfig({
  ...
  server: {
    ...
    strictPort: true,
    /**
     * `@penka/backoffice-api` registers no CORS plugin, so a browser on 5174
     * cannot call 3001 directly — every request fails before it is sent.
     * Proxying keeps the dev server same-origin with the admin API instead,
     * which is also how the console is served in an environment that puts both
     * behind one host.
     *
     * `.env.development` empties `VITE_ADMIN_API_BASE_URL` so the client builds
