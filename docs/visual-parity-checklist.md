...
| 14 | — | **Competencia** panel above the status panel: a league `select` and a fecha segmented control | The prototype worked on one hard-coded matchday. The console guessed both values instead — the league from the first penka listed, the number as `resolvedMatchdays + 1` — which left penkas on a second league unreachable and walked off the end of a finished league into `matchday_not_found`. Both now come from data the API sent: `GET /admin/v1/penkas` and `GET /admin/v1/leagues/:leagueId/matchdays` |
